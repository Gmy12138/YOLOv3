...
if __name__ == "__main__":
    ...
    train_path = '/home/ecust/gmy/YOLOv3/data/NEU-DET/train/image'
    valid_path = '/home/ecust/gmy/YOLOv3/data/NEU-DET/valid/image'
    ...
    # dataset1 = ListDataset(valid_path, augment=True, multiscale=opt.multiscale_training)
    ...
    for epoch in range(opt.epochs):
        ...
        # total_loss = 0
        # for batch_i, (_, imgs, targets) in enumerate(dataloader1):
        #     batches_done = len(dataloader1) * epoch + batch_i
        #
        #     # imgs = Variable(imgs.to(device))
        #     # targets = Variable(targets.to(device), requires_grad=False)
        #     model.eval()
        ...
        #     targets = targets.to(device)
        #
        #     loss1, outputs = model(imgs, targets)
        #
        ...
        if epoch % opt.checkpoint_interval == 0 and (AP.mean())>best_map:#取最好的map保存
            torch.save(model.state_dict(), f"checkpoints/yolov3_ckpt_%d.pth" % (epoch+1))
            best_map = AP.mean()
